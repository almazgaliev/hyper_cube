...
class HyperCubeAnimation():
    ...
    def update_frame(self, frame, *fargs):
        ...
        rotation = create_transform_arr([
            rotate_arr(angle, dim=5, axes=[4, 3, 2], use_proj=True),

            rotate_arr(angle, dim=5, axes=[4, 3, 1], use_proj=True),
            rotate_arr(angle, dim=5, axes=[4, 3, 0], use_proj=True),

            rotate_arr(angle, dim=5, axes=[4, 2, 0], use_proj=True),
            rotate_arr(angle, dim=5, axes=[4, 1, 0], use_proj=True),

            rotate_arr(angle, dim=5, axes=[3, 2, 1], use_proj=True),
            rotate_arr(angle, dim=5, axes=[3, 2, 0], use_proj=True),
            rotate_arr(angle, dim=5, axes=[3, 1, 0], use_proj=True),
            # rotate_arr(angle, dim=5, axes=[2, 1, 0], use_proj=True),
        ])
